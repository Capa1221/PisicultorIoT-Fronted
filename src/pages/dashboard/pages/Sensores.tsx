...
const Sensores = () => {
  ...
  const [newSensor, setNewSensor] = useState<NewSensor>({
    idHibernadero: sessionStorage.getItem("idHibernadero") || "", // Initialize with value from sessionStorage
    nombre: "",
    descripcion: "",
    config: false,
  });
  ...
  const handleGuardarNuevo = async () => {
    try {
      ...
      sessionStorage.removeItem("idHibernadero"); // Clear idHibernadero from sessionStorage
    } catch (error) {
  ...
  const handleSelectHibernadero = (id: string) => {
    ...
    sessionStorage.setItem("idHibernadero", id)
    setNewSensor((prevSensor) => ({
      ...prevSensor,
      idHibernadero: id,
    }));
